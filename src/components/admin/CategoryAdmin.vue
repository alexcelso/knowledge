...
<div class="category-admin">
    <input id="category-admin" type="hidden" v-model="category.id">
    <b-form>  
          <b-form-group 
            label="Nome:" label-for="category-name" >
            <b-form-input 
              id="category-name" 
              type="text" 
              v-model="category.name" 
              :readonly="mode === 'remove'" 
              required placeholder="Informe o Nome da Categoria."/>
          </b-form-group>           
          <b-form-group 
    ...
    <b-table hover striped :items="categories" :fields="fields">
      <template slot="actions" slot-scope="data">
        <b-button variant="danger" @click="loadCategory(data.item)" class="mr-2">
          <i class="fa fa-pencil"></i>
        </b-button>
       <button variant="danger" @click="loadCategory(data.item, 'remove')">
